...
export default function Contact() {
  ...
  // Los que estén vacíos en el YAML no se muestran.
  const channels = [
    ...socialLinks,
    profile.phone
      ? { id: 'phone', icon: 'phone', label: profile.phone, url: `tel:${profile.phone.replace(/\s/g, '')}` }
      : null,
    profile.location ? { id: 'location', icon: 'location', label: profile.location, url: '' } : null,
  ].filter(Boolean)
          ...
          <Reveal variants={scaleIn} delay={0.08} className="flex flex-col gap-4">
            ...
            <ul className="flex flex-col gap-2">
              {channels.map((channel) => {
                ...
                const content = (
                  <>
                    <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-line bg-surface text-accent-ink shadow-inner-top">
                    ...
                    {channel.url ? (
                      <ICONS.arrowUpRight
                        aria-hidden="true"
                        className="ml-auto h-4 w-4 shrink-0 text-ink-subtle transition-colors duration-200 group-hover:text-accent-ink"
                      />
                    ) : null}
                  </>
                )
