  ...
  return (
    <Section id="education">
      ...
      <div className="mt-12 grid gap-4 lg:mt-16 lg:grid-cols-2">
        {(data.programs ?? []).map((program, index, programs) => {
          const Icon = getIcon(program.icon)
          // Con un número impar de titulaciones, la última se estira para no
          // quedarse sola en media fila.
          const cierraFila = index === programs.length - 1 && programs.length % 2 === 1
          return (
            <Reveal
              key={`${program.institution}-${program.program}`}
              variants={scaleIn}
              delay={index * 0.07}
              className={`min-w-0 ${cierraFila ? 'lg:col-span-2' : ''}`}
            >
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex min-w-0 flex-col gap-1.5">
                    ...
                    <p className="text-sm text-ink-muted">
                      {/* La ubicación es opcional: sin ella no queda un «·» colgando. */}
                      {[program.institution, program.location].filter(Boolean).join(' · ')}
                    </p>
                  </div>
                  <span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-line bg-surface text-accent-ink shadow-inner-top transition-colors duration-200 group-hover:border-line-accent">
        ...
        <div className="mt-16">
          <Reveal>
            <h3 className="mb-6 flex items-center gap-2.5 text-2xl font-semibold tracking-tight text-ink md:text-3xl">
              <ICONS.award aria-hidden="true" className="h-6 w-6 text-accent-ink" />
            ...
            {data.certifications.map((certification, index) => {
              ...
              >
                <p className="flex items-baseline justify-between gap-3 text-sm font-medium text-ink">
                  ...
                  <span className="shrink-0 font-mono text-[0.6875rem] text-accent-ink">
                    {certification.year}
                  </span>
                </p>
