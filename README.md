...
Diseño: *Linear / Modern* en **rojo y negro** — fondo ambiental por capas, focos que siguen al
...
src/
  ...
  sections/
    ...
    About/
      About.jsx
      content.es.md  content.en.md          ← perfil, pilares y foto del espacio de trabajo
    Experience/
      Experience.jsx
    ...
    Testimonials/
      Testimonials.jsx
      content.es.yaml  content.en.yaml      ← referencias y testimonios
...
- **Referencias y testimonios**: cada persona sin `quote` se muestra como
  tarjeta de referencia (foto, nombre, cargo y vínculo). En cuanto le pegues su
  reseña en `quote`, pasa a ser tarjeta de testimonio. Sus teléfonos se guardan
  en el YAML pero nunca se publican.
- **Hoja de vida**: el botón flotante ofrece los PDF de `public/cv/` que
  existan (`Esteban-Jordan-CV.pdf` y, si lo añades, `Esteban-Jordan-CV-ATS.pdf`).
- **Colores**: el rojo de acento y los pozos de luz del fondo son tokens en
  `src/index.css`; al final del archivo hay paletas alternativas listas para
  copiar.
- **Proyectos**: el campo `size` (`hero`, `tall`, `wide`) controla el tamaño de
  la tarjeta en la grilla; `links` puede quedar vacío si el proyecto no es público.
